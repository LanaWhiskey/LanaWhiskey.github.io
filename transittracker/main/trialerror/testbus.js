...
center: [-77.03675, 38.89388], // starting position [lng, lat]
...
async function run(){
	...
	locations.forEach((bus, i) => {
		...
		.setLngLat([bus.attributes.Lon, bus.attributes.Lat])
		.setPopup(new mapboxgl.Popup({offset: 25, closeOnClick: false, closeButton: false}).setHTML(`<h3>Route ID <br>
		${bus.attributes.RouteID}</h3>`))
		.addTo(map)
		.togglePopup();
...
async function getBusLocations(){
	const url = 'https://api.wmata.com/Bus.svc/json/jBusPositions?RouteID=70?api_key=6aed38a9c1f84aca94ece11b721413c5';
