...
center: [-71.077014, 42.397641], // centered position [lng, lat]
...
async function run(){
	...
	locations.forEach((bus, i) => {
		...
		.setPopup(new mapboxgl.Popup({offset: 0, closeOnClick: false, closeButton: false}).setHTML(`<h3>Route: ${bus.relationships.route.data.id}<br>Bus ID: ${bus.attributes.label}<br>Speed ${bus.attributes.speed}</h3>`))
		.addTo(map)
		.togglePopup();
